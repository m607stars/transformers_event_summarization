...
class BartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.event_attention = BartAttention(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True,
        )
        self.event_attn_layer_norm = nn.LayerNorm(self.embed_dim)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            residual = hidden_states
            event_cross_attn_past_key_value = past_key_value[2:4] if past_key_value is not None else None
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        ...
        self.generalized_pooling = GeneralizedPooling()
        # Initialize weights and apply final processing
        self.post_init()
